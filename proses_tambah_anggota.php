...
if($_POST){
	...
	if(empty($nama)) {
		...
		include "koneksi.php";
		$insert=mysqli_query($conn,"insert into anggota (nama, tanggal_lahir, jenis_kelamin, alamat, username, password, id_barang) value ('".$nama."', '".$tanggal_lahir."','".$jenis_kelamin."','".$alamat."','".$username."','".$password."','".$id_barang."')") or die(mysqli_error($conn)); 
		if($insert){
			echo"<script>alert('Sukses menambah data Anggota');location.href='media.php';</script>";
		} else {
			echo "<script>alert('Gagal menambahkan data Anggota');location.href='media.php';</script>";
}
